...
struct BitmapMemory {
    ~BitmapMemory() {
        if (phy_addr != 0 && vir_addr != nullptr) {
            (void)AXCL_SYS_MemFree(phy_addr, vir_addr);
        }
    }
...
bool PrepareBitmap(const PipelineOsdBitmap& bitmap, PreparedBitmap* prepared_bitmap) {
    ...
    if ((row_bytes % 16U) != 0U) {
        std::fprintf(stderr, "pipeline osd axcl: bitmap row bytes must be 16-byte aligned, got %zu\n", row_bytes);
        return false;
    }

    auto memory = std::make_shared<BitmapMemory>();
    const auto* token = reinterpret_cast<const AX_S8*>("PipelineOsdBmp");
    if (AXCL_SYS_MemAlloc(&memory->phy_addr, &memory->vir_addr, static_cast<AX_U32>(byte_size), kBitmapMemoryAlignment,
                          token) != AX_SUCCESS) {
        return false;
    }

    if (axclrtMemcpy(memory->vir_addr, bitmap.data.data(), byte_size, AXCL_MEMCPY_HOST_TO_DEVICE) != AXCL_SUCC) {
        return false;
    }
...
class PreparedAxclPipelineOsd final : public PreparedPipelineOsd {
public:
    explicit PreparedAxclPipelineOsd(std::uint32_t hold_frames) : hold_frames_(hold_frames) {}
    ...
    bool Apply(common::AxImage& image) const override {
        ...
        if (!mosaics_.empty()) {
            ...
            if (AXCL_IVPS_DrawMosaicTdp(&frame, mosaics.data(), static_cast<AX_U32>(mosaics.size())) != AX_SUCCESS) {
                return false;
            }
        }
        ...
        if (!bitmaps_.empty()) {
            ...
            if (AXCL_IVPS_DrawOsdTdp(&frame, attrs.data(), static_cast<AX_U32>(attrs.size())) != AX_SUCCESS) {
                return false;
            }
        }
        ...
        for (const auto& line : lines_) {
            ...
            if (AXCL_IVPS_DrawLine(&canvas, attr, ax_points.data(), static_cast<AX_U32>(ax_points.size())) != AX_SUCCESS) {
                return false;
            }
        }
        ...
        for (const auto& polygon : polygons_) {
            ...
            if (AXCL_IVPS_DrawPolygon(&canvas, attr, ax_points.data(), static_cast<AX_U32>(ax_points.size())) !=
                AX_SUCCESS) {
                return false;
            }
        }
...
class AxclPipelineOsdRenderer final : public PipelineOsdRenderer {
public:
    std::shared_ptr<const PreparedPipelineOsd> Prepare(const PipelineOsdFrame& frame) override {
        auto prepared = std::make_shared<PreparedAxclPipelineOsd>(frame.hold_frames);
...
std::unique_ptr<PipelineOsdRenderer> CreatePlatformPipelineOsdRenderer() {
    return std::make_unique<AxclPipelineOsdRenderer>();
}
